...
//        text = "";
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode == MeasureSpec.EXACTLY) {
            ...
            if (!TextUtils.isEmpty(regex)) {
                height = 0;
                String[] texts = getText().toString().split(regex);
                for (String s : texts) {
                    height = Math.max(height, getDesiredHeight(s));
                }
                height += letterSpacing;
            } else {
                height = getDesiredHeight(getText().toString());
            }
        ...
        if (!TextUtils.isEmpty(regex)) {
            ...
            getFormatTexts(getText().toString());
        }
        ...
        if (widthMode == MeasureSpec.EXACTLY) {
            ...
            if (!TextUtils.isEmpty(regex)) {
                ...
                int desiredH = getDesiredHeight(getText().toString());
                int measureH = height - getPaddingTop() - getPaddingBottom();
                if (measureH == 0){
                    measureH = 1; // divide by zero
                }
    ...
    private void getFormatTexts(String s) {
        ...
        if (contentHeight > 0 && getDesiredHeight(s) > contentHeight) {
            int count = contentHeight / charHeight;
            int i = 0;
            for (; i < getDesiredHeight(s) / contentHeight; i++) { // divide by zero
