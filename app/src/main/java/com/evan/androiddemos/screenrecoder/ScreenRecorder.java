...
package com.evan.androiddemos.screenrecoder;
...
public class ScreenRecorder {
    ...
    public interface Callback {
        ...
        void onRecording(long presentationTimeUs,byte[] bytes);
    }
    ...
    private void record() {
        ...
        try {
            ...
            Log.e("evan","record mDstPath = "+mDstPath);
            // create encoder and input surface
            prepareVideoEncoder();
            prepareAudioEncoder();
        } catch (IOException e) {
//            throw new RuntimeException(e);
            Log.e("evan","IOException");
            e.printStackTrace();
        }
    ...
    private void writeSampleData(int track, MediaCodec.BufferInfo buffer, ByteBuffer encodedData) {
        ...
        if (encodedData != null) {
            ...
            if (mCallback != null) {
                ...
                mCallback.onRecording(buffer.presentationTimeUs, bytes);
            }
//            mMuxer.writeSampleData(track, encodedData, buffer);
    ...
    private void resetVideoOutputFormat(MediaFormat newFormat) {
        ...
        if (mCallback != null) {
            ...
            byte[] bytes = new byte[sps.remaining()]; //ByteBuffer转化为byte[]
            Log.i(TAG, "Sent sps " + bytes.length);
            sps.get(bytes);
            mCallback.onRecording(0, bytes);

            ByteBuffer pps = newFormat.getByteBuffer("csd-1");//pps
            bytes = new byte[pps.remaining()]; //ByteBuffer转化为byte[]
